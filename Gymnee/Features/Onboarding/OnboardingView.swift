...
/// ゲストは価値を体験してから必要な場面（ソーシャル等）でサインインする。後からのサインイン時は
/// IdentityAdoptionPolicy がゲスト期間の記録だけを新 userId へ引き継ぐ（恒久アカウントの
/// データを別アカウントが吸い上げることはない。docs/identity-environment-design.md）。
