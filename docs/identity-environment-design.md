...
- `Config/Secrets.{dev,prod}.xcconfig` に `GYMNEE_ENV = dev|prod` を追加、Info.plist 経由で読む。
- 起動時アサート: bundle id サフィックス（`.dev` 有無）から**期待 env** を導き、`GYMNEE_ENV` および
  接続先 host と一致しなければ**リモート同期を無効化してローカルのみで動作**（＋ログ）。
  - 特に Release ビルド（無印 bundle）が dev host に繋ぐ状態を起動時に遮断する。
- デモ/シード人格生成（`-gymneeDemo` ハーネス等）が `#if DEBUG` に閉じており prod 接続ストアへ
  書き込めないことを確認・保証する。
...
- ~~F1 の実装方式: 段階的か一気か~~ → **決定（2026-07-10）: 一気に Phase 2（匿名認証）へ移行する。**
  実装確定事項は §6「Phase 2 実装確定事項」を参照。
