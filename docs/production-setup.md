...
```
このスクリプトが流すもの：
- **スキーマ**：`supabase/setup_all.sql`（19+テーブル / RLS / Storage バケット / `delete_account` RPC / モデレーション / プッシュトリガー `0008`）
...
- **Edge Function**：`send-push` を prod へ deploy ＋ secrets（APNS_*、`APNS_HOST=api.push.apple.com`、新規 `PUSH_SHARED_SECRET`）
